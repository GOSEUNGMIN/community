...
public class MemberController {
    ...
    @PostMapping("/insert")
    public String insert(@ModelAttribute @Valid MemberDto memberDto, BindingResult result, HttpSession session) {
    ...
    public String login(@ModelAttribute LoginDto loginDto, Model model, HttpSession session) {
        ...
        if (success) {
            session.setAttribute("loginDto", loginDto); // 로그인 성공시 세션 저장
            return "redirect:/"; // 성공 시 메인페이지 이동
        } else {
